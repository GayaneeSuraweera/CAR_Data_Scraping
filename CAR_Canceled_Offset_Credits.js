...
async function getCanceled_offset_credits() {
  let driver = await new Builder().forBrowser("chrome").build();
  try {
    await driver.get(
      "https://thereserve2.apx.com/myModule/rpt/myrpt.asp?r=308"
    );
    while (true) {
      ...
      for (let i = 5; i < rows.length - 4; i++) {
        try {
          ...
          const cancellation_reason = await cells[13].getText();
          ...
          await saveJSONToFile(
            ...
            `./canceled_offset_credits_details/${projectId}.json`
          );
        } catch (error) {
